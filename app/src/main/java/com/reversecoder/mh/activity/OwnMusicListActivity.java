...
import com.reversecoder.mh.adapter.OwnMusicListViewAdapter;
import com.reversecoder.mh.model.Music;
import com.reversecoder.mh.model.ResponseOwnMusic;
...
import com.reversecoder.mh.util.IntentManager;
...
import static com.reversecoder.mh.util.AllConstants.INTENT_KEY_OWN_MUSIC_LIST_ITEM_MUSIC;
...
public class OwnMusicListActivity extends AppCompatActivity {
    ...
    private void initOwnMusicListUI() {
        ...
        isFromMenu = data.getBooleanExtra(INTENT_KEY_OWN_MUSIC_LIST_FROM_MENU, false);

        if (isFromMenu) {
            intentUser = data.getParcelableExtra(INTENT_KEY_OWN_MUSIC_LIST_ITEM_USER);
            Log.d(TAG, "user: " + intentUser);
        } else {
            mMusic = data.getParcelableExtra(INTENT_KEY_OWN_MUSIC_LIST_ITEM_MUSIC);
