package com.reversecoder.mh.service;
...
import static com.reversecoder.mh.util.AllConstants.DEFAULT_PAID_PLAYBACK;
import static com.reversecoder.mh.util.AllConstants.EXTRA_ACTION_START;
import static com.reversecoder.mh.util.AllConstants.EXTRA_ACTION_STOP;
import static com.reversecoder.mh.util.AllConstants.INTENT_FILTER_ACTIVITY_UPDATE;
import static com.reversecoder.mh.util.AllConstants.KEY_INTENT_EXTRA_ACTION;
import static com.reversecoder.mh.util.AllConstants.KEY_INTENT_EXTRA_MUSIC;
...
public class MediaService extends Service {
    ...
    private void sendUpdateToActivity(Music music) {
        broadcastIntentActivityUpdate.putExtra(KEY_INTENT_EXTRA_MUSIC_UPDATE, music);
    ...
    private void updateSeekProgress() {
        if (audioPlayer != null) {
            if (audioPlayer.isPlaying()) {
                if (music.getIs_paid().equalsIgnoreCase("1") && AppUtils.isPlayedFor(audioPlayer.getCurrentPosition(), DEFAULT_PAID_PLAYBACK)) {
                    audioPlayer.pause();

                    music.setIsPlaying(MEDIA_PLAYBACK_PAID);
                    sendUpdateToActivity(music);
                } else {
                    music.setIsPlaying(MEDIA_PLAYER_RUNNING);
